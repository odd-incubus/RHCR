...
TASKS_FILE = 'centre_10_tasks.txt' # Adjusted path for potential subfolder
...
def parse_map(filepath):
    ...
    with open(filepath, 'r') as f:
        lines = f.readlines()
        # lines[0] is "Grid size (x, y)" - a descriptive header
        # lines[1] contains the actual dimensions like "77,37"
        grid_dim_parts = lines[1].strip().split(',')
        grid_dim = (int(grid_dim_parts[0]), int(grid_dim_parts[1]))
        
        # lines[2] is the data header: id,type,station,x,y,...
        # Node data starts from lines[3]
        for line in lines[3:]: # Skip first three lines (description, dimensions, header)
            parts = line.strip().split(',')
            if not parts or not parts[0]: # Skip empty or malformed lines
                continue
            node_id = int(parts[0])
            node_type = parts[1]
            station_str = parts[2]
        ...
        for i in range(1, num_agents + 1):
            ...
            if len(parts) > 1 and parts[1]: # Check if there are tasks
                ...
                for task_str in task_strings:
                    ...
                    if not task_parts[0]: continue # Skip if location_id is empty

                    location_id_str = task_parts[0]
                    
                    # Check if this is the final task marker (e.g., "location_ID,-1,")
                    if len(task_parts) > 1 and task_parts[1] == "-1":
                        final_node_id = int(location_id_str)
                        
                        task_info = {
                            'start_node_id': current_node_id,
                            'end_node_id': final_node_id,
                            'arrival_time': time_ready_for_travel, # Agent arrives at its final destination
                            'task_duration': 0, # Stays here (effectively infinite for simulation)
                            'is_final': True
                        }
                        agents_data[agent_id]['tasks'].append(task_info)

                        # Path update for the move to the final node
                        # Agent was at current_node_id, moves to final_node_id, arriving at time_ready_for_travel
                        _start_x_final, _start_y_final = nodes_map[current_node_id]['x'], nodes_map[current_node_id]['y']
                        end_x_final, end_y_final = nodes_map[final_node_id]['x'], nodes_map[final_node_id]['y']
                        
                        # Agent is at its final position from time_ready_for_travel onwards.
                        agents_data[agent_id]['path'][time_ready_for_travel] = (end_x_final, end_y_final)
                        if time_ready_for_travel > max_time:
                            max_time = time_ready_for_travel
                        
                        current_node_id = final_node_id # Update current node
                        break # Done with tasks for this agent
                    
                    # Else, it's a regular task, requires all three parts (loc, arrival, duration)
                    if len(task_parts) < 3:
                        print(f"Warning: Agent {agent_id}, malformed task string '{task_str}'. Skipping task.")
                        continue
                        
                    end_node_id = int(location_id_str)
                    arrival_time = int(task_parts[1])
                    # Ensure task_duration string is not empty before converting
                    if not task_parts[2]:
                        print(f"Warning: Agent {agent_id}, task '{task_str}' has empty duration. Assuming 0.")
                        task_duration = 0
                    else:
                        task_duration = int(task_parts[2]) # This is now safer
                    ...
                    # Add these lines to define end_x, end_y
